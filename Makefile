...
release:
	@ $(MAKE) clean
	@ rm -rf bird
	@ $(MAKE) -f birdy.mk NAME=bird MODE=release HEADERS_DIR=src/includes SOURCE_DIR=src
...
debug:
	@ $(MAKE) clean
	@ rm -rf birdd
	@ $(MAKE) -f birdy.mk NAME=birdd MODE=debug HEADERS_DIR=src/includes SOURCE_DIR=src
