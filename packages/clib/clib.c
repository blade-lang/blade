...
static inline b_ffi_values *get_c_values(b_vm *vm, b_ffi_cif *cif, b_obj_list *list) {
  ...
  for(int i = 0; i < list->items.count; i++) {
    add_value(values, switch_c_values(vm, cif->arg_types[i]->as_int, list->items.values[i], cif->arg_types[i]->as_ffi->size));
  }
...
DECLARE_MODULE_METHOD(clib_new_struct) {
  ...
  if(args_list->items.count > 0) {
    elements = ALLOCATE(ffi_type *, args_list->items.count + 1);
    clib_types = ALLOCATE(int, args_list->items.count);

    for(int i = 0; i < args_list->items.count; i++) {
      elements[i] = AS_PTR(args_list->items.values[i])->pointer;
    }
...
DECLARE_MODULE_METHOD(clib_create_struct) {
  ...
  if(args_list->items.count > 0) {
    values->values = ALLOCATE(void *, args_list->items.count);
    for(int i = 0; i < args_list->items.count && types[i] != NULL; i++) {
      add_value(values, switch_c_values(vm, type_ptr->types[i], args_list->items.values[i], types[i]->size));
    }
  }

  CLIB_RETURN_PTR(values, <void *clib::structvalue(%d)>, args_list->items.count);
}
