...
/**
 * Maximum value that "should" exist in a list passed to UUInt16Array.
 * @type number
 */
...
/**
 * class UInt16Array represents an array of twos-complement 16-bit signed 
...
class UInt16Array {
  ...
  UInt16Array(n) {
    if is_number(n) {
      ...
      for item in n {
        if !is_number(item) {
          raise ValueError('invalid UInt16Array value')
        }
      }
  ...
  set(index, value) {
    ...
    if !is_number(value) and !is_int(value)
      raise ArgumentError('UInt16Array stores integer values')
  ...
  extend(array) {
    if !instance_of(array, UInt16Array)
      raise TypeError('instance of UInt16Array expected')
   ...
   */
  clone() {
    return UInt16Array(self.to_list())
  }
