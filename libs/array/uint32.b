...
/**
 * class UInt32Array represents an array of twos-complement 16-bit signed 
...
class UInt32Array {
  ...
  UInt32Array(n) {
    if is_number(n) {
      ...
      for item in n {
        if !is_number(item) {
          raise ValueError('invalid UInt32Array value')
        }
      }
  ...
  set(index, value) {
    ...
    if !is_number(value) and !is_int(value)
      raise ArgumentError('UInt32Array stores integer values')
  ...
  extend(array) {
    if !instance_of(array, UInt32Array)
      raise TypeError('instance of UInt32Array expected')
   ...
   */
  clone() {
    return UInt32Array(self.to_list())
  }
