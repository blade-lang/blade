...
def pack(format, ...) {
  if !is_string(format)
    raise ArgumentError('format string expected')
...
def pack_from(format, args) {
  if !is_string(format)
    raise TypeError('format string expected in argument 1')
  if !is_list(args)
    raise TypeError('list expected in argument 2 (args)')
